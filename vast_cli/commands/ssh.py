...
@parser.command(
    ...
    aliases=hidden_aliases(["attach ssh"]),
    usage="vastai instance attach-ssh instance_id ssh_key",
    ...
    epilog=deindent("""
        ...
        Examples:
         vastai instance attach-ssh "ssh 12371 ssh-rsa AAAAB3NzaC1yc2EAAA..."
         vastai instance attach-ssh "ssh 12371 ssh-rsa $(cat ~/.ssh/id_rsa)"
    """),
)
def instance__attach_ssh(args):
...
@parser.command(
    ...
    aliases=hidden_aliases(["detach ssh"]),
    usage="vastai instance detach-ssh instance_id ssh_key_id",
    help="Detach an ssh key from an instance",
    epilog=deindent("""
        Example: vastai instance detach-ssh 99999 12345
    """)
)
def instance__detach_ssh(args):
...
@parser.command(
    ...
    aliases=hidden_aliases(["create ssh-key"]),
    usage="vastai ssh-key create [ssh_public_key] [-y]",
...
@parser.command(
    ...
    aliases=hidden_aliases(["delete ssh-key"]),
    usage="vastai ssh-key delete ID",
    help="Remove an ssh-key",
)
def ssh_key__delete(args):
...
@parser.command(
    aliases=hidden_aliases(["show ssh-keys"]),
    usage="vastai ssh-key list",
    help="List your ssh keys associated with your account",
)
def ssh_key__list(args):
    url = apiurl(args, "/ssh/")
...
@parser.command(
    ...
    aliases=hidden_aliases(["update ssh-key"]),
    usage="vastai ssh-key update ID SSH_KEY",
    help="Update an existing SSH key",
)
def ssh_key__update(args):
...
@parser.command(
    ...
    aliases=hidden_aliases(["ssh-url"]),
    usage="vastai instance ssh-url ID",
    help="ssh url helper",
)
def instance__ssh_url(args):
    """
...
@parser.command(
    ...
    aliases=hidden_aliases(["scp-url"]),
    usage="vastai instance scp-url ID",
    help="scp url helper",
)
def instance__scp_url(args):
    """
