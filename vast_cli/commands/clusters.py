...
@parser.command(
    ...
    aliases=hidden_aliases(["create cluster"]),
    usage="vastai cluster create SUBNET MANAGER_ID",
    help="Create Vast cluster",
    epilog=deindent("""
        Create Vast Cluster by defining a local subnet and manager id.""")
)
def cluster__create(args: argparse.Namespace):
...
@parser.command(
    ...
    aliases=hidden_aliases(["create overlay"]),
    usage="vastai overlay create CLUSTER_ID OVERLAY_NAME",
...
def overlay__create(args: argparse.Namespace):
    json_blob = {
...
@parser.command(
    ...
    aliases=hidden_aliases(["delete cluster"]),
    usage="vastai cluster delete CLUSTER_ID",
    help="Delete Cluster",
    epilog=deindent("""
        Delete Vast Cluster""")
)
def cluster__delete(args: argparse.Namespace):
    json_blob = {
        "cluster_id": args.cluster_id
    }
...
@parser.command(
    ...
    aliases=hidden_aliases(["delete overlay"]),
    usage="vastai overlay delete OVERLAY_IDENTIFIER",
    help="Deletes overlay and removes all of its associated instances"
)
def overlay__delete(args: argparse.Namespace):
...
@parser.command(
    ...
    aliases=hidden_aliases(["join cluster"]),
    usage="vastai cluster join CLUSTER_ID MACHINE_IDS",
    help="Join Machine to Cluster",
    epilog=deindent("""
        Join's Machine to Vast Cluster
    """)
)
def cluster__join(args: argparse.Namespace):
    json_blob = {
        "cluster_id": args.cluster_id,
...
@parser.command(
    ...
    aliases=hidden_aliases(["join overlay"]),
    usage="vastai overlay join OVERLAY_NAME INSTANCE_ID",
    help="Adds instance to an overlay network",
    epilog=deindent("""
    Adds an instance to a compatible overlay network.""")
)
def overlay__join(args: argparse.Namespace):
    json_blob = {
        "name": args.name,
...
@parser.command(
    aliases=hidden_aliases(["show connections"]),
    usage="vastai cluster connections [--api-key API_KEY] [--raw]",
    help="Display user's cloud connections"
)
def cluster__connections(args):
    """
...
@parser.command(
    aliases=hidden_aliases(["show clusters"]),
    usage="vastai cluster list",
    help="Show clusters associated with your account.",
    epilog=deindent("""
        Show clusters associated with your account.
    """)
)
def cluster__list(args: argparse.Namespace):
...
@parser.command(
    aliases=hidden_aliases(["show overlays"]),
    usage="vastai overlay list",
    help="Show overlays associated with your account.",
    epilog=deindent("""
        Show overlays associated with your account.
    """)
)
def overlay__list(args: argparse.Namespace):
    req_url = apiurl(args, "/overlay/")
    r = http_get(args, req_url, headers=state.headers)
    r.raise_for_status()
    response_data = r.json()
...
@parser.command(
    ...
    aliases=hidden_aliases(["remove-machine-from-cluster"]),
    usage="vastai cluster remove-machine CLUSTER_ID MACHINE_ID NEW_MANAGER_ID",
...
def cluster__remove_machine(args: argparse.Namespace):
    json_blob = {
        "cluster_id": args.cluster_id,
